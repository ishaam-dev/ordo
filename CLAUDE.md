...
- `src/analyzer.ts` — serial worker, one harness run per thread, tool access handed in by core, 5-call budget, writes `analyses` incl. the session id chat forks. An `api-key` harness doesn't start the loop without `COPILOT_HARNESS_SPEND_OK=1`
