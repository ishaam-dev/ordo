...
user, shows them as one feed prioritized by urgency, where each thread is pre-analyzed by
Claude (why it matters, summary, suggested action, context pulled from the user's local
MCP servers), and lets the user discuss any thread with Claude and send a reply back to
Slack from an explicit button click. Everything runs on the user's Mac.

This document describes what is built. `README.md` is the user-facing guide, `docs/ux.md`
is the UI spec, `CLAUDE.md` is the working guide for agents.
...
```
 ...
 Workspace B ──Socket Mode──┤        conversations.list / .history / .replies
                  ...
                  ┌──────────────────────────────────────┐
                  │ Analyzer — Claude Agent SDK, serial, │
                  │ read-only MCP tools, one thread/run  │
                  └──────────┬───────────────────────────┘
   ...
   │ GET/POST /api/thread/:id/chat  (SSE stream, Agent SDK resume/fork) │
...
| `analyses` | one row per thread: urgency, why, summary, suggested_action, context_notes, `covered_through_ts`, `analyzed_at`, `session_id` (the analyzer's Agent SDK session, which chat forks) |
...
## 4. Analyzer (`src/analyzer.ts`)

A serial worker: 15s tick, a thread must be quiet for 45s before it is picked up, 5-minute
backoff after a failure, one `query()` at a time, 180s hard timeout, ≤8 turns. It writes
...
- Prompt: workspace/channel/kind, the user's identity, and the transcript inside explicit
  ...
  rather than "Claude never answered".
- **Read-only, enforced three ways**, because thread text is attacker-controlled input:
  `tools: []` (no built-in tools at all) + `disallowedTools`, a `canUseTool` gate that allows
  only `mcp__*` names not matching a mutation-word regex and caps the run at 5 tool calls,
  and a `PreToolUse` hook that fires even for tools a user setting auto-allows.
- `settingSources: ['user']` — the user's global Claude Code config and their MCP servers,
  not this repo's project settings. An analysis is not a coding session.
- Subprocess env is sanitized: `SLACK_*` (real user tokens must never reach the child) and
  `CLAUDE*` / `ANTHROPIC_BASE_URL` (nested-session markers that exist when the server was
  itself launched from inside a Claude Code session, and which make the spawned CLI defer
  auth to a host session that is not there).
...
## 5. Chat and the send path (`src/chat.ts`)
...
- **Session handling:** resume our own stored session (`chat_sessions`) if there is one;
  otherwise **fork** the analyzer's session for that thread (`forkSession: true`) so chat
  starts already knowing the thread and its reasoning while the analyzer's transcript stays
  intact; otherwise start fresh with transcript + analysis seeded into the first prompt. A
  resume that fails falls back to the seeded path exactly once — and only when the *session*
  looks like the problem, never for auth/budget/rate-limit/timeout.
...
- **Sending is deliberately not a model tool**, and there is no draft id. The bytes posted to
  ...
  touching Slack.
- Same read-only enforcement as the analyzer, 240s timeout, ≤12 turns, 8 tool calls/turn.
  One in-flight turn per thread; a second POST is refused, not queued.
...
## 6. Health and failure reporting (`src/health.ts`, `GET /api/status`)
...
`unknown`, each with a plain-English message, a hint, and an exact command when a command is
the fix. Classification is structural where we control the throw site (`ClassifiedError`) and
text-matching for SDK/CLI strings — that regex list is maintenance-sensitive, and a real
failure showing up as `unknown` means new wording to add.
...
- Model sessions are read-only three ways and their subprocess env is stripped of `SLACK_*`.
  Nothing is ever posted to Slack without a click. Nothing logs message text, draft text or
  tokens — the reply audit line records destination and character count only.
...
| Agent SDK for analysis *and* chat | One integration; reuses the local Claude Code login, MCP config, tool loop, resumable sessions |
| Chat forks the analyzer's session | Starts knowing the thread and its verdict, without corrupting the analyzer's transcript |
...
analyzer with read-only MCP context, health reporting, the three-pane web UI, the chat panel
with draft-and-confirm replies, and the packaged Mac app with menubar, notifications and
autostart.

Not built, and deliberately so for now: multi-user or any auth beyond the localhost token,
auto-send of anything, a settings UI, search over history, a test suite, and notarised
distribution (the bundle is ad-hoc signed, hence the first-open dialog documented in
`README.md`). Known UI gaps are listed at the end of `docs/ux.md`.
